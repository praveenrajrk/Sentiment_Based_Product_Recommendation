...
class SentimentRecommenderModel:
    ...
    ROOT_PATH = "pickle/"
    MODEL_NAME = "sentiment-classification-log-reg-model.pkl"
    VECTORIZER = "tfidf-vectorizer.pkl"
    RECOMMENDER = "user_final_rating.pkl"
    CLEANED_DATA = "cleaned-text-data.pkl"
    ...
    def getTop20RecommendationByUser(self, user_name):
        top20_recommendations = list(self.user_final_rating.loc[user_name].sort_values(ascending=False)[0:20].index)
        return top20_recommendations

    # defining a method to filter only top5 recommendations based on sentiment analysis model
    def getSentimentBasedRecommendations(self, user_name):
        if user_name in self.user_final_rating.index:
            ...
            X = self.vectorizer.transform(
                filtered_data["reviews_lemmatized_text"].values.astype(str))
